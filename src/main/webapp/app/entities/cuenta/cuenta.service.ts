...
export class CuentaService {
  ...
  protected convertDateFromClient(cuenta: ICuenta): ICuenta {
    const copy: ICuenta = Object.assign({}, cuenta, {
      fechaBaja: cuenta.fechaBaja && cuenta.fechaBaja.isValid() ? cuenta.fechaBaja.format(DATE_FORMAT) : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.fechaBaja = res.body.fechaBaja ? moment(res.body.fechaBaja) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((cuenta: ICuenta) => {
        cuenta.fechaBaja = cuenta.fechaBaja ? moment(cuenta.fechaBaja) : undefined;
      });
    }
    return res;
  }
