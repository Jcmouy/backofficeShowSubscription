...
export class CuentaComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.cuentaService
      ...
      .subscribe(
        (res: HttpResponse<ICuenta[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInCuentas(): void {
    this.eventSubscriber = this.eventManager.subscribe('cuentaListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ICuenta[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
