...
export class PagoService {
  ...
  protected convertDateFromClient(pago: IPago): IPago {
    const copy: IPago = Object.assign({}, pago, {
      fechaExterna: pago.fechaExterna && pago.fechaExterna.isValid() ? pago.fechaExterna.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.fechaExterna = res.body.fechaExterna ? moment(res.body.fechaExterna) : undefined;
    }
    return res;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((pago: IPago) => {
        pago.fechaExterna = pago.fechaExterna ? moment(pago.fechaExterna) : undefined;
      });
    }
    return res;
  }
